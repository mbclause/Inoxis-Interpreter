...
Description: given an arithmetic expression exp, add the proper instructions for adding the operands in the correct
order, preserving left-right associativity. Effectively, we will be doing an in-order traversal of a binary tree
*/
void   compileArithmetic(expression  exp, GArray* instructions, GHashTable* jumpLabels, GArray* functionLocations)
{
	// base case, we hit either a literal expression, a function call, or a unary expression
	// basically, anything that's not another binary expression
	if (exp.kind != EXPR_BIN)
	{
		// compile the non binary expression
		// this will ultimately push a number operand onto the stack
		compileExpression(exp, instructions, jumpLabels, functionLocations);

		return;
	}

	// treat the subscript operator as a unary operator in this context
	if (exp.val.binaryOp.op == BRACKETS)
	{		
		// compile the non binary expression
		// this will ultimately push a number operand onto the stack
		compileExpression(exp, instructions, jumpLabels, functionLocations);
	...
	compileArithmetic(*exp.val.binaryOp.lhs, instructions, jumpLabels, functionLocations);

	// visit node
	compileExpression(exp, instructions, jumpLabels, functionLocations);

	// traverse right subtree
	compileArithmetic(*exp.val.binaryOp.rhs, instructions, jumpLabels, functionLocations);
